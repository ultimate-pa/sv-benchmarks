...
void reach_error() { __assert_fail("0", "popl20-queue-add-3.wvr.c", 21, __extension__ __PRETTY_FUNCTION__); }
...
int *q1, *q2;
int q1_front, q1_back, q2_front, q2_back, s, t, n1, n2, N;
...
void* thread1() {
  for (int i=0; i<N; i++) {
    ...
    assume_abort_if_not(q1[q1_back] == 1);
    q1_back++;
...
void* thread2() {
  ...
  while (1) {
    ...
    assume_abort_if_not(q2[q2_back] == j);
    q2_back++;
    __VERIFIER_atomic_end();
    s = s + j;
    if (j == 0) {
